...
def Build_RC_Column(Diameter,Height_of_Column, fPrimeC,dbi, dti, CLl, dblc, nb,cover, Ablc, CLt, Atc, dbtc, s_tc, datadir, AxialLoad, GM_file, GM_dt, GM_npt):
    # -----------------------------------------------------------------------------
    #
    #    ^Y
    #    |
    ...
    Weight = AxialLoad * kips  # superstructure weight
    ...
    with open(datadir + "\\mat.out", 'w') as matfile:
        matfile.write("%s %s %s %s %s %s %s %s %s %s %s %s %s %s \n" % (
        Fy, fyt, Ast, st, Dprime, PCol, DCol, LCol, barAreaSec, fc, SPsy, SPsu, SPb, SPR))
    matfile.close
    ...
    recorder('Element', '-file', datadir + '/StressStrain4.out', '-time', '-ele', 2, 'section', '1', 'fiber',
             str(-Rbl), '0', '3', 'stressStrain')  # Rbl,0, IDreinf
