...
def Postprocessor_of_data(GM_fn, CL,D,SF,ALR,rhol,rhov):
    # 1. Opening folder to acces data

    SpectrumDir=r'C:\ConditionDependentPBEE\NLTHA_ConditionDependentPBEE\MainshocksParallel_1.2.1\ResponseSpectrumAnalysis'
    rootdir = r'C:\ConditionDependentPBEE\NLTHA_ConditionDependentPBEE\MainshocksParallel_1.2.1\data'
    ...
    datadir = rootdir + '\\' + GM_fn + "\\CL" + str(CL) + "\\D"+str(D)+"\\SF"+str(SF)+"\\ALR"+str(ALR)+"\\RhoL"+str(rhol)+"\\Rhov"+str(rhov)
    ...
    Teff = (2*np.pi)*(np.sqrt((meff/Keff)))
    ...
    # 12. Writing CSV File
    DataFrame_Out.to_csv(r'C:\ConditionDependentPBEE\NLTHA_ConditionDependentPBEE\MainshocksParallel_1.2.1\results\PosprocData.csv', mode='a', header=False)
